...
export default function rootReducer(state = initialState, action) {
    if(action.type === "GET_DOG_API"){
      ...
      switch(action.payload){
        case "A-Z" :
          return {...state, dogs: state.dogs.sort((a,b)=> {return a.name > b.name ? 1 : -1}), filDogs: state.filDogs.sort((a,b)=> {return a.name > b.name ? 1 : -1})}
        case "Z-A" : 
          return {...state, dogs: state.dogs.sort((a,b)=> {return b.name > a.name ? 1 : -1}), filDogs: state.filDogs.sort((a,b)=> {return b.name > a.name ? 1 : -1})}
        case "Peso mayor" : 
