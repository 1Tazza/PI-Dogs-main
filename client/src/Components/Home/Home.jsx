...
return(<div>
  <div className={`${c.justcenter} ${c.column}`}>
   ...
   <div className={c.margin}>
     <select className={c.select} disabled={disable} id="order" onChange={(e) => handleOrderChange(e)}>
   ...
   <div>
      <select className={c.select} disabled={disable || (filDogs.length > 0 && filDogs.every(e => e.temperament === null))} id="filter" onChange={(e) => handleFilterChange(e)}>
   ...
   <br />
    {dogs.length > 0 && state.isLoading === false && !disable ? <Paginado totalItems={filDogs.length > 0 ? filDogs.length : dogs.length} itemsPerPage={itemsPerPage}/> : null}
    
    </div>
    </div>
   {state.isLoading ? <img src={logo} alt="logo"/> : <div className={`${c.row} ${c.container}`}> {actualPage.map( el => <Card key={el.id} id={el.id} name={el.name} img={el.img} temperament={el.temperament} weight={el.weight} />)} </div>}
